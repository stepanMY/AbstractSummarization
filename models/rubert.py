# https://huggingface.co/IlyaGusev/rubert_telegram_headlines
import torch
...
class ruBERT:
    """
    Class for russian ruBERT model
    https://huggingface.co/IlyaGusev/rubert_telegram_headlines
    """
    def __init__(self,
                 xtokenize_params,
                 ...
                 path_to_model='IlyaGusev/rubert_telegram_headlines',
        ...
        """
        self.device = device
        self.model = EncoderDecoderModel.from_pretrained(path_to_model).to(self.device)
    ...
    def tokenize_x(self, x):
        ...
        """
        x_tok = self.tokenizer(x, return_tensors='pt', **self.xtokenize_params)
        return x_tok
    ...
    def generate(self, x):
        ...
        with torch.no_grad():
            x_tok = self.tokenize_x(x).to(self.device)
            preds_tok = self.model.generate(input_ids=x_tok.input_ids,
                                            attention_mask=x_tok.attention_mask,
